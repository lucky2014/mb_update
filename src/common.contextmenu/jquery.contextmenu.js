  ...
  jQuery.fn.contextPopup = function(menuData) {
    ...
    function createMenu(e) {
      var menu = $('<ul class="' + settings.contextMenuClass + '"><div class="' + settings.gutterLineClass + '"></div></ul>')
        ...
        //点点逻辑begin
        var target = $(e.target);
        /*if(target.closest(".elementLi").length!=0){

        }else{
          if(target.closest(".wordEdit").length==0&&target.closest(".assembly").length==0&&target.closest(".contextMenuPlugin").length==0){
            $(".changeDiv").addClass("dn");
            $(".elementLi").removeClass("keyupDiv");
            $(".textMain").removeAttr("contenteditable");
            $(".noMove").removeClass("noMove");
            $(".editLi").removeClass("editLi");
            $(".wordEdit").addClass("dn");
            $(".assembly").addClass("dn");
            $(".deleteBtn").addClass("noClick");
            $(".moveUp").addClass("noClick");
            $(".moveDown").addClass("noClick");
          }
        }
      $(".keyupDiv").siblings(".textLi").find(".textMain").attr("contenteditable",false);*/
      //end
      if (settings.title) {
