define(function(require,exports,module){
    ...
    var box = Engine.init();
    require("common.activeX/companyProductSet/index.css");
    ...
    var app = {
        ...
        img_edit: function(v){
            ...
            var img_showTpl = require("common.activeX/companyProductSet/index.tpl");
            box.render($(".right"), v, img_showTpl);

            $.each(v.attrList, function(i,j){
                if(j.tagName == "input"){
                    $(".companyProductSet .modelInput").val(j.attrTitle)
                    j.text = 1; //用以区分
                }else if(j.tagName == "img"){
                    var picsTpl = require("common.activeX/companyProductSet/pics.tpl");
                     box.render($(".companyProductSet .productsPics ul"), v.attrList, picsTpl);
                }
            });
            ...
            for(var i=0;i<$(".companyProductSet .piculCommon .hasPics").length;i++){ //获取轮播图片
                imgArr.push($(".companyProductSet .piculCommon .hasPics").eq(i).find(".addPicture").attr("src"))
            }
             //上传图片
             $(".rightDiv").delegate("input[name=myfiles]", "change", function() {
                 ...
                 ajaxFileUpload(id, "uploadImg.do", function(msg){
                     ...
                     if($("#" + idd ).parents("li").hasClass("nodataPics")){
                            ...
                            var len = $(".companyProduct .swiper-wrapper").find(".swiper-slide").length/3;
                            $(".companyProduct .swiper-wrapper").find(".swiper-slide").eq(_index+len).css('backgroundImage','url('+msg[0]+')');
            ...
            $(".rightDiv").delegate(".delectCha","click",function(){ //删除
                ...
                $.each($(".piculCommon li"), function(i){ //删除图片后更新id和class
                    $(this).find("input[name=myfiles]").attr("id","companyProduct"+i);
                });
                 $(".companyProduct").find(".selectB").css("height",$(".companyProduct").height()-4);
                 pubsub.publish('dataChange',"product_show");
            });
